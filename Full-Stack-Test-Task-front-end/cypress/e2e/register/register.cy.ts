...
describe("Register Page", () => {
  ...
  it("should navigate user to register if clicked on Sing Up option", () => {
    ...
    cy.get(register.errorList).should("contain", "Name must be at least 3 characters.");
    cy.get(register.errorList).should("contain", "Password must be at least 8 characters, include at least one letter, one number, and one special character.");
  });
});